...
async function loadProducts() {
  try {
    products = await FW.getProducts();
    renderProducts(products);
  } catch(e) {
    console.error('Failed to load products:', e);
    $('#productsGrid').innerHTML = '<p style="grid-column:1/-1;text-align:center;color:var(--muted);padding:60px 0;font-weight:600;">couldn\'t load products — try refreshing</p>';
  }
}
...
function renderProducts(items) {
  ...
  const merch = items.filter(p => !callSlugs.includes(p.slug));

  if (!merch.length) {
    grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:var(--muted);padding:60px 0;font-weight:600;">merch coming soon...</p>';
    return;
  }
  ...
  merch.forEach(p => {
    ...
    card.innerHTML = `
      <div class="p-card-img">
        ${img ? `<img src="${img.url}" alt="${p.name}" loading="lazy">` : '<div style="width:100%;height:100%;background:#eee"></div>'}
      </div>
...
function buildVariants(p) {
  ...
  if (sizes.length > 0) {
    ...
    sizes.forEach(name => {
      const b = document.createElement('button');
      b.className = 'opt-btn';
      b.textContent = name; b.dataset.size = name;
...
function renderCart() {
  ...
  items.forEach(item => {
    ...
    el.querySelector('.ci-rm').addEventListener('click', async e => {
      await FW.removeFromCart(e.target.dataset.vid);
      await refreshCart();
    });
...
function setupCalls() {
  ...
  calls.forEach(call => {
    ...
    tier.className = 'call-tier';
    tier.innerHTML = `
      <div>
        <p class="call-tier-name">${call.label}</p>
        <p class="call-tier-price">${call.price || ''}</p>
      </div>
      <button class="btn btn-pop btn-sm tier-buy">Book</button>
    `;
...
// =====================
function fmt(m) {
  return m ? `$${m.value % 1 === 0 ? m.value : m.value.toFixed(2)}` : '';
}
